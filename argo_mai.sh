...
argoCert() {
	[ $cloudflaredStatus == "未安装" ] && red "检测到未安装CloudFlare Argo Tunnel客户端，无法执行操作！！！" && exit 1
	[ $loginStatus == "未登录" ] && red "请登录CloudFlare Argo Tunnel客户端后再执行操作！！！" && exit 1
	sed -n "1, 5p" /root/.cloudflared/cert.pem >>/usr/local/share/au/server.key
	sed -n "6, 24p" /root/.cloudflared/cert.pem >>/usr/local/share/au/server.crt
	green "CloudFlare Argo Tunnel证书提取成功！"
	yellow "证书crt路径如下：/usr/local/share/au/server.crt"
	yellow "私钥key路径如下：/usr/local/share/au/server.key"
	green "使用证书提示："
