...
getSingleCert(){
    ...
    if [[ -n $(echo $domainIP | grep nginx) ]]; then
        ...
        if [[ $domainIP == $ipv4 ]]; then
            bash ~/.acme.sh/acme.sh --issue -d ${domain} --standalone -k ec-256 --server $certsrv
        fi
        if [[ $domainIP == $ipv6 ]]; then
            bash ~/.acme.sh/acme.sh --issue -d ${domain} --standalone -k ec-256 --server $certsrv --listen-v6
        fi
...
getDomainCert(){
    ...
    if [[ -z $ipv4 ]]; then
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "*.${domain}" -d "${domain}" -k ec-256 --server $certsrv --listen-v6
    else
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "*.${domain}" -d "${domain}" -k ec-256 --server $certsrv
    fi
    bash ~/.acme.sh/acme.sh --install-cert -d "*.${domain}" --key-file /root/private.key --fullchain-file /root/cert.crt --ecc
    checktls
}
...
getSingleDomainCert(){
    ...
    export CF_Email="$CFemail"
    if [[ -z $ipv4 ]]; then
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "${domain}" -k ec-256 --server $certsrv --listen-v6
    else
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "${domain}" -k ec-256 --server $certsrv
    fi
    bash ~/.acme.sh/acme.sh --install-cert -d "${domain}" --key-file /root/private.key --fullchain-file /root/cert.crt --ecc
    checktls
}
...
menu() {
    ...
    green "2. 申请单域名证书（80端口申请） （Let's Encrypt）"
    green "3. 申请单域名证书（80端口申请） （ZeroSSL）"
    green "4. 申请单域名证书（CF API申请） （Let's Encrypt）"
    green "5. 申请单域名证书（CF API申请） （ZeroSSL）"
    green "6. 申请泛域名证书（CF API申请） （Let's Encrypt）"
    green "7. 申请泛域名证书（CF API申请） （ZeroSSL）"
    green "8. 撤销并删除已申请的证书"
    green "9. 手动续期域名证书"
    green "10. 卸载Acme.sh域名证书申请脚本"
    green "0. 退出"
    echo "         "
    ...
    case "$NumberInput" in
        1) install_acme ;;
        2) getSingleCert ;;
        3) certsrv="zerossl" && getSingleCert ;;
